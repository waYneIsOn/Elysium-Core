...
#define ELYSIUM_CORE_TEMPLATE_DIAGNOSTICS_STACKTRACE_WINDOWS
...
namespace Elysium::Core::Template::Diagnostics
{
	...
	{
	public:
		inline constexpr StackTrace(const System::uint16_t MaximumFramesToCapture = DefaultFramesToCapture) noexcept
			: _StackFrames(CaptureStackFrames(MaximumFramesToCapture))
		{ }

		constexpr StackTrace(const StackTrace& Source) = delete;

		constexpr StackTrace(StackTrace&& Right) noexcept = delete;

		constexpr ~StackTrace() noexcept = default;
	public:
		constexpr StackTrace& operator=(const StackTrace& Source) = delete;

		constexpr StackTrace& operator=(StackTrace&& Right) noexcept = delete;
	public:
		inline const Elysium::Core::Template::Container::Vector<StackFrame>& GetFrames() const noexcept
		{
			return _StackFrames;
		}
	private:
		inline Elysium::Core::Template::Container::Vector<StackFrame> CaptureStackFrames(const System::uint16_t MaximumFramesToCapture) noexcept
		{
			{
...
#error "Unknown architecture"
	...
	public:
		static constexpr const Elysium::Core::Template::System::uint16_t DefaultFramesToCapture = 0xFFFF;
	private:
		static constexpr const Elysium::Core::Template::System::uint16_t MaxFunctionNameLength = 1024;
	private:
		Elysium::Core::Template::Container::Vector<StackFrame> _StackFrames;
	};
}
#endif
#endif
