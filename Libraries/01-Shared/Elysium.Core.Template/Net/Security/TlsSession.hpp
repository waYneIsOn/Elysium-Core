...
namespace Elysium::Core::Template::Net::Security
{
	...
	public:
		inline const Elysium::Core::Template::IO::ReadResult ReadBlock(Elysium::Core::Template::Container::View::Span<Elysium::Core::Template::System::byte>& DataView)
		{
			...
			InBufferDescriptor.pBuffers = InBuffers;
			...
			while(true)
			{
				...
				InBuffers[0].cbBuffer = _InBuffer.GetLength();
	...
	private:
		...
		inline void PerformClientHandshake(SecBuffer* RenegotiationBuffer)
		{
			...
			while(true)
			{
				...
				const bool IsFirstCall = FirstCall;

				if (!IsFirstCall && nullptr != RenegotiationBuffer)
				{
				...
				Status = InitializeSecurityContextW(&_CredentialHandle, IsFirstCall && nullptr == RenegotiationBuffer ? nullptr : &_ContextHandle, &TargetHostUTF16LE[0], RequiredContext, 0,
					SECURITY_NATIVE_DREP, IsFirstCall ? nullptr : &InputBuffersDesc, 0, &_ContextHandle, &OutputBufferDescriptor, &ContextAttributes, &Lifetime);
				FirstCall = false;
				...
				case SEC_E_OK:
				{
					if (!IsFirstCall)
					{
						// Schannel has just processed the current input.
						// Preserve anything it says it didn't consume.
		...
		{
			for (ULONG i = 0; i < OutputBufferDescriptor.cBuffers; ++i)
			{
