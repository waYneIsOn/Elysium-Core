...
Elysium::Core::size Elysium::Core::Data::SqlNativeClient::OleDb::SqlNativeCommand::ExecuteNonQuery()
{
	...
	{
		throw SqlNativeException(HResult, _NativeCommandFactory);
	}

	// set the command text
	Elysium::Core::Container::VectorOfByte Bytes = _WindowsEncoding.GetBytes(&_Text[0], _Text.GetLength(), sizeof(wchar_t));
...
void Elysium::Core::Data::SqlNativeClient::OleDb::SqlNativeCommand::Prepare()
{
	...
	if (FAILED(HResult = NativeCommandPrepare->Prepare(0)))
	{
		...
		SqlNativeException Exception = SqlNativeException(HResult, NativeCommandPrepare);
...
void Elysium::Core::Data::SqlNativeClient::OleDb::SqlNativeCommand::PrepareParameters(ICommandText* NativeCommandText, DBPARAMS * CommandParameters, std::vector<ISequentialStream*>* Streams, std::vector<byte>* ParameterDataBuffer)
{
	...
	if (ParameterCount > 0)
	{
				...
				if (!_Parameters[i].GetIsDBNull())
				{
					...
					if (SourceStream != nullptr && (void*)SourceStream != &Elysium::Core::Data::DBNull::GetValue())
					{
						...
						while ((BytesRead = SourceStream->Read(&Buffer[0], 4096)) > 0)
						{
							...
							{
								SqlNativeException Exception = SqlNativeException(HResult, CurrentStream);
								NativeCommandWithParameters->Release();
								NativeCommandText->Release();
								throw Exception;
							}
		...
		if (FAILED(HResult))
		{
			SqlNativeException Exception = SqlNativeException(HResult, NativeParameterAccessor);
