...
#include "../../../../Elysium-Core/Libraries/01-Shared/Elysium.Core.Template/FixedSizeArray.hpp"
...
namespace UnitTests::Core::Template::Container
{
	...
	public:
		TEST_METHOD(Constructors)
		{
			...
			FixedSizeArray<uint32_t, 3> InitializerListSmallerInstance = { 1, 2 };
			...
			FixedSizeArray<uint32_t, 3> InitializerListLargerInstance = { 1, 2, 3, 4 };
			...
			FixedSizeArray<uint32_t, 3> InitializerListInstance = { 1, 2, 4 };
			...
			FixedSizeArray<uint32_t, 5> Instance = FixedSizeArray<uint32_t, 5>();
			Instance.GetAt(0) = 10;
			Instance.GetAt(1) = 11;
			Instance.GetAt(2) = 12;
			Instance.GetAt(3) = 13;
			...
			// Copy constructor
			FixedSizeArray<uint32_t, 5> CopiedInstance = FixedSizeArray<uint32_t, 5>(Instance);
			...
			// Move constructor
			FixedSizeArray<uint32_t, 5> MovedInstance = FixedSizeArray<uint32_t, 5>(Move(CopiedInstance));
			...
			FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>> MAllocInstance = FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>>();
			...
			FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>> CopiedMAllocInstance = FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>>(MAllocInstance);
			...
			FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>> MovedMAllocInstance = FixedSizeArray<uint32_t, 3, MAllocator<uint32_t>>(Move(CopiedMAllocInstance));
		...
		{
			FixedSizeArray<uint32_t, 3> Instance = { 1, 2, 3 };
	...
	private:
		...
		void Iterations(FixedSizeArray<uint32_t, Size>& Instance)
		{
